...
const output = path.join(root, '.work', selected.size?'targeted-review':'slide-review', slug);
...
(async()=>{
  ...
  const report={slug,slides:count,selected:[...selected],views:[],errors,failed};
  for(const size of [{width:1280,height:720},{width:1920,height:1080}]){
    await page.setViewportSize(size);
    await page.evaluate(()=>Reveal.layout());
    for(let i=0;i<count;i++){
      if(selected.size&&!selected.has(await page.evaluate(i=>Reveal.getSlides()[i].id,i)))continue;
      ...
      const metrics=await page.evaluate(()=>{
        ...
        const foot=s.querySelector('.slide-foot').getBoundingClientRect();
        const footerOverlap=[...s.querySelectorAll('h1,h2,p,pre,td,strong,.recall-answers')].filter(n=>!n.closest('.notes,.slide-foot')&&getComputedStyle(n).visibility!=='hidden'&&getComputedStyle(n).display!=='none').filter(n=>{const b=n.getBoundingClientRect();return b.height&&b.bottom>foot.top-10&&b.top<foot.bottom}).map(n=>n.textContent.slice(0,90));
        return {overflow,footerOverlap, missingImages:[...s.querySelectorAll('img')].filter(i=>!i.complete||!i.naturalWidth).map(i=>i.getAttribute('src')),fontLoaded:document.fonts.check('48px Hermes'),notesHidden:[...s.querySelectorAll('.notes')].every(n=>getComputedStyle(n).display==='none')};
      });
  ...
  report.ok=errors.length===0&&failed.length===0&&report.views.every(v=>v.overflow.length===0&&v.footerOverlap.length===0&&v.missingImages.length===0&&v.fontLoaded&&v.notesHidden)&&report.speaker.controlWorks&&report.helpOpens;
  fs.writeFileSync(path.join(output,'report.json'),JSON.stringify(report,null,2)+'\n');
  console.log(JSON.stringify({slug,slides:count,ok:report.ok,errors,failed,issues:report.views.filter(v=>v.overflow.length||v.footerOverlap.length||v.missingImages.length||!v.fontLoaded),speaker:report.speaker.controlWorks,output}));
