"""Build the public handbook and lecture views from reviewed UTF-8 content.
...
def learn(course):
    ...
    for ci,ch in enumerate(chapters):
        slides=[]
        for si,s in enumerate(ch['slides']):
            paragraphs=''.join(f'<p>{inline(t)}</p>' for t in s.get('paragraphs',[]))
            checkpoints='<ul class="checkpoints">'+''.join('<li>'+inline(x)+'</li>' for x in s.get('checkpoints',[]))+'</ul>' if s.get('checkpoints') else ''
            related='<div class="related">'+''.join(link(x['href'],x['label']) for x in s.get('links',[]))+'</div>'
            slides.append(f'''<section class="lesson-slide" id="{esc(s['id'])}" data-slide="{si}" aria-label="{esc(s['title'])}"><div class="slide-heading"><span class="eyebrow">{ci+1:02d} / {si+1:02d} · {inline(ch['title'])}</span><h2>{inline(s['title'])}</h2><p class="claim">{inline(s['claim'])}</p></div><div class="slide-evidence">{visual(s.get('visual'),ch['id'])}{codeblock(s.get('code'))}</div><div class="reading-body">{paragraphs}{checkpoints}{related}<p class="transition">{inline(s.get('transition',''))}</p></div><details class="speaker-notes"><summary>발표자 설명</summary><p>{inline(s.get('notes',''))}</p></details></section>''')
        nav=''.join(f'<a {"aria-current=page" if x["id"]==ch["id"] else ""} href="{esc(x["id"])}.html">{i+1:02d} {inline(x["title"])}</a>' for i,x in enumerate(chapters))
        contents=''.join(f'<a href="#{esc(x["id"])}">{inline(x["title"])}</a>' for x in ch['slides'])
        next_ch=chapters[ci+1] if ci+1<len(chapters) else chapters[0]
        next_deck=f'{next_ch["id"]}.html?present=1#{next_ch["slides"][0]["id"]}' if ci+1<len(chapters) else ''
        prev_ch=chapters[ci-1] if ci else None
        prev_deck=f'{prev_ch["id"]}.html?present=1#{prev_ch["slides"][-1]["id"]}' if prev_ch else ''
        body=f'''<div class="course-tools"><a href="index.html">← 전체 강의</a><div><button id="present-toggle" aria-pressed="false">발표 보기</button><button id="print-page">인쇄</button></div></div><div class="course-layout"><aside class="sidebar"><details open><summary>강의 순서</summary><nav>{nav}</nav></details></aside><main id="main" class="course-main"><header class="chapter-intro"><span class="eyebrow">CHAPTER {ci+1:02d} / {len(chapters):02d}</span><h1>{inline(ch['title'])}</h1><p class="lead">{inline(ch['description'])}</p><nav class="on-page" aria-label="이 장의 순서">{contents}</nav></header>{"".join(slides)}<a class="next-chapter" href="{next_ch['id']}.html"><span>다음으로 이어 읽기</span><strong>{inline(next_ch['title'])} →</strong></a></main></div><div class="presentation-controls" hidden><button id="prev-slide" aria-label="이전 슬라이드">←</button><span id="slide-count" role="status"></span><button id="next-slide" aria-label="다음 슬라이드">→</button><button id="notes-toggle" aria-pressed="false">설명</button><button id="exit-present">읽기로 돌아가기 Esc</button></div>'''
        body=body.replace('class="course-main"',f'class="course-main" data-next-deck="{esc(next_deck)}" data-prev-deck="{esc(prev_deck)}"')
        write(f'learn/{ch["id"]}.html',shell(ch['title'],body,kind='course',description=ch['description']))
    rows=''.join(f'<a class="chapter-row" href="{esc(ch["id"])}.html"><span class="chapter-number">{i+1:02d}</span><div><h2>{inline(ch["title"])}</h2><p>{inline(ch["description"])}</p></div><span class="row-meta">{len(ch["slides"])}개 주제 <b>↗</b></span></a>' for i,ch in enumerate(chapters))
    body=f'<main id="main" class="course-index"><div class="eyebrow">THE OPEN COURSE</div><h1>시스템을 이해하는<br>일곱 번의 탐구</h1><p class="lead">한 요청을 따라 다섯 분야의 구조와 기술을 배웁니다.<br>차분히 읽거나, 도해와 함께 발표할 수 있습니다.</p><p class="scope-note">교육용 사례와 공개 코드의 동작을 사용합니다. 실제 Hermes의 구조는 위키에서 함께 확인하세요.</p><div class="chapter-list">{rows}</div>{sources(course.get("sources",[]))}</main>'
    write('learn/index.html',shell('강의',body))
...
def home():
    ...
    body=f'''<main id="main"><section class="home-hero"><div><div class="eyebrow">AN OPEN GUIDE TO AGENT SYSTEMS</div><h1>에이전트가 일을<br><span>이어가는 구조</span></h1><p class="lead">에이전트가 일을 이어가는 구조를 탐구합니다.<br>작업·지식·카탈로그·이미지·영상을<br>하나의 연결된 시스템으로 읽어 보세요.</p><div class="hero-actions"><a class="primary-link" href="learn/start.html">첫 강의 시작하기 <span>→</span></a><a href="wiki/start.html">위키에서 살펴보기 ↗</a></div><p class="hero-note">Hermes 시스템 해설과 독립적으로 실행하는 공개 도구</p></div><div class="system-map"><div class="map-caption">FIVE DOMAINS / ONE SYSTEM</div><svg viewBox="0 0 560 430" aria-hidden="true"><path d="M115 135H440M280 135V275M115 275H440M115 135V275M440 135V275" fill="none" stroke="#aebdea" stroke-width="2" stroke-dasharray="5 7"/><circle cx="280" cy="205" r="28" fill="#3455ed"/><path d="M269 205H291M284 198L291 205L284 212" stroke="white" fill="none" stroke-width="2"/></svg>{system}<div class="map-foot">목표 · 근거 · 선택 · 산출물</div></div></section><section class="home-path"><div><span class="eyebrow">CHOOSE YOUR PATH</span><h2>어디서 시작할까요?</h2></div><a href="learn/index.html"><span class="path-icon">01</span><h3>흐름을 따라 배우기</h3><p>하나의 사례로 원리를 익히고<br>다섯 분야의 기술을 연결합니다.</p><span>강의 보기 →</span></a><a href="wiki/reference.html"><span class="path-icon">02</span><h3>직접 확인하며 만들기</h3><p>실행 가능한 공개 코드와<br>입출력 계약을 살펴봅니다.</p><span>코드 시작하기 →</span></a></section><section class="home-domains"><div><span class="eyebrow">EXPLORE THE SYSTEM</span><h2>다섯 분야,<br>서로 다른 책임</h2><p>각 분야를 개념, 구조, 동작,<br>기술과 검증으로 나누어 설명합니다.</p></div><div>{rows}</div></section><section class="home-case"><div><span class="eyebrow">A SHARED EXAMPLE</span><h2>짧은 요청 하나에<br>어떤 판단이 담길까요?</h2><p>책상용 램프의 소개 자료를 만든다고 생각해 보세요. 제품의 특징을 확인하고, 구성을 선택하고, 장면을 연결하는 동안 시스템의 역할이 드러납니다.</p><a href="learn/start.html">LUMA 사례 따라가기 ↗</a></div><figure>{lamp()}<figcaption>교육용으로 구성한 램프와 화면 배치</figcaption></figure></section></main>'''
...
def main():
    ...
    wp=ROOT/'content/wiki/pages.json';cp=ROOT/'content/slides/course.json'
    if wp.exists():wiki(json.loads(wp.read_text(encoding='utf-8-sig'))['pages'])
    if cp.exists():learn(json.loads(cp.read_text(encoding='utf-8-sig')))
    home()
    write('.nojekyll','')
    # Preserve archived preview deep links and give the old entry a clear path forward.
    write('lectures/index.html',shell('강의 안내','<main id="main" class="course-index"><h1>p-hermes 강의</h1><p class="lead">완성된 새 강의에서 전체 시스템을 탐구하세요.</p><a class="primary-link" href="../learn/index.html">전체 강의 보기 →</a><details class="archive-links"><summary>이전 시안 기록</summary><a href="prototype.html">첫 시안</a><a href="redesign/index.html">상호작용 시안</a><a href="course-preview/index.html">읽기 시안</a></details></main>'))
